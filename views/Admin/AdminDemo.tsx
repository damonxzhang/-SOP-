...
const AdminDashboard: React.FC = () => {
  ...
  const [inquiries, setInquiries] = useState<StepInquiry[]>([])
  ...
  useEffect(() => {
    // 并行获取所有数据
    Promise.all([
      fetch('/backend/sop/list', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ page: 1, limit: 100 })
      }).then((r) => r.json()),
      fetch('/backend/users/list').then((r) => r.json()),
      fetch('/backend/devices/list', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ page: 1, limit: 100 })
      }).then((r) => r.json()),
      fetch('/backend/inquiries/list', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ page: 1, limit: 100 })
      }).then((r) => r.json()),
      fetch('/backend/media/list', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ page: 1, limit: 100 })
      }).then((r) => r.json()),
      fetch('/backend/auth/current-user').then((r) => r.json())
    ])
      .then(
        ([
          guidesRes,
          ...
          if (guidesRes.code === 200 && guidesRes.data?.list) {
          ...
          if (devicesRes.code === 200 && devicesRes.data?.list) {
            setDevices(devicesRes.data.list)
          }
          if (inquiriesRes.code === 200 && Array.isArray(inquiriesRes.data)) {
            setInquiries(inquiriesRes.data)
          }
          if (mediaRes.code === 200 && Array.isArray(mediaRes.data)) {
            setMediaResources(mediaRes.data)
          }
            ...
            <div className='space-y-4'>
              <button className='w-full flex items-center justify-between p-4 bg-slate-50 hover:bg-slate-100 rounded-2xl transition-colors group'>
                <div className='flex items-center'>
                  <Lock
                    size={16}
                    className='mr-3 text-slate-400 group-hover:text-blue-600'
                  />
        ...
        <div className='lg:col-span-2 space-y-6'>
          <div className='bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm'>
            <div className='flex items-center justify-between mb-8'>
              ...
              <button className='px-6 py-2 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg active:scale-95 transition-all'>
                保存变更
              </button>
            </div>
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
              ...
              <div className='md:col-span-2 space-y-2'>
                ...
                <textarea
                  rows={4}
                  className='w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl text-xs font-bold outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all shadow-inner resize-none'
                  defaultValue='负责系统全局维护与 SOP 标准化流程制定，拥有 10 年以上半导体设备维保经验。'
                />
              </div>
            </div>
          </div>
          ...
          <div className='bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm'>
            ...
            <div className='space-y-6'>
              {[
                {
                  action: '审核通过了 SOP 规程',
                  target: 'NXT:2050i 激光校准 SOP',
                  time: '2小时前',
                  icon: <CheckCircle2 className='text-emerald-500' />
                },
                {
                  action: '修改了工程师权限',
                  target: '王技术员 (Role: Senior)',
                  time: '5小时前',
                  icon: <Users className='text-blue-500' />
                },
                {
                  action: '删除了过期多媒体资料',
                  target: 'OLD_GUIDE_V1.PDF',
                  time: '昨天 14:20',
        ...
        {[
          ...
          },
          {
            ...
            value: inquiries
              ...
              .length.toString(),
            trend: 'High',
            icon: <MessageCircleCode className='text-amber-600' />,
            color: 'bg-amber-50'
          },
          {
            label: '高危操作占比',
          ...
          <div
            ...
            <div
              className={`w-12 h-12 ${stat.color} rounded-2xl flex items-center justify-center mb-4`}>
              {stat.icon}
            </div>
  ...
  const renderInquiries = () => (
    <div className='space-y-6 animate-in fade-in duration-500'>
      <div className='bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6'>
        <div className='flex items-center space-x-5'>
          <div className='p-4 bg-amber-500 text-white rounded-[1.5rem] shadow-2xl shadow-amber-100 rotate-1'>
          ...
          <div>
            ...
            <p className='text-sm text-slate-500'>
              实时记录工程师在一线维保过程中的疑问与现场物证，快速响应专家闭环
            </p>
          </div>
        </div>
        <div className='flex flex-col md:flex-row items-center gap-4'>
          ...
          <div className='flex items-center space-x-3'>
            <span className='px-4 py-2 bg-amber-50 text-amber-600 rounded-xl text-[10px] font-black border border-amber-100 uppercase tracking-widest'>
          ...
          <table className='w-full text-left border-collapse'>
            ...
            <tbody className='divide-y divide-slate-100'>
              {filteredInquiries.map((inq) => {
                const engineer = users.find((u) => u.id === inq.engineerId)
                const device = devices.find((d) => d.id === inq.deviceId)
                const guide = guides.find((g) => g.id === inq.guideId)
                return (
                  <tr
                    key={inq.id}
                    ...
                    <td className='px-4 py-6'>
                      <span className='px-3 py-1 bg-slate-100 text-slate-700 rounded-lg text-[10px] font-black font-mono'>
                        {inq.isNewIssue
                          ? inq.context?.faultCode || 'N/A'
                          : guide?.faultCode || inq.context?.faultCode || 'N/A'}
                      </span>
                    </td>
                    <td className='px-4 py-6'>
                      <div className='flex flex-col'>
  ...
  const renderUserManagement = () => (
    <div className='space-y-6 animate-in fade-in duration-500'>
      <div className='bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6'>
        ...
        <button className='px-8 py-3.5 bg-slate-900 text-white rounded-2xl font-black text-sm flex items-center shadow-2xl hover:bg-indigo-600 transition-all active:scale-95'>
    ...
    return (
      <div className='space-y-6 animate-in fade-in duration-500'>
        <div className='bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6'>
          ...
          <button
            onClick={() =>
              setUploadingMedia({ name: '', tags: [], type: 'image' })
            }
          ...
          <div className='bg-white w-full max-w-4xl h-[80vh] rounded-[3.5rem] shadow-2xl flex flex-col overflow-hidden border border-white/20'>
              ...
              <div className='grid grid-cols-12 gap-10'>
                <div className='col-span-5 space-y-6'>
                    ...
                    <div className='space-y-4'>
                      <div>
                        ...
                        <p className='text-xs font-black text-slate-800'>
                          {
                            users.find(
                              (u) => u.id === viewingInquiry.engineerId
                            )?.name
                          }
                        </p>
                      </div>
                      <div>
                        ...
                        <p className='text-xs font-black text-slate-800'>
                          {
                            devices.find(
                              (d) => d.id === viewingInquiry.deviceId
                            )?.model
                          }{' '}
                          (
                          {
                      ...
                      <div>
                        ...
                        <p className='text-xs font-black text-slate-800'>
                          {viewingInquiry.context?.stepTitle ||
                  ...
                  {viewingInquiry.photoUrl && (
                    <div className='bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-4'>
                      <h4 className='text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50 pb-2 flex items-center'>
                        <FileImage size={12} className='mr-2' /> 现场物证照片
                      </h4>
                      <img
                        src={viewingInquiry.photoUrl}
                ...
                <div className='col-span-7 space-y-6'>
                  ...
                  <div className='bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm space-y-4'>
                    ...
                    <textarea
                      className='w-full p-4 bg-slate-50 border border-slate-200 rounded-3xl text-xs min-h-[160px] outline-none focus:ring-2 focus:ring-blue-500 transition-all'
                      placeholder='请输入指导意见，该内容将同步推送给现场工程师移动端...'
                    />
                  </div>
                </div>
              </div>
            ...
            <div className='px-10 py-6 border-t border-slate-100 flex items-center justify-between bg-white'>
              <div className='flex items-center space-x-2 text-[10px] font-black text-slate-400 uppercase tracking-widest'>
                <BadgeCheck className='text-emerald-500' size={14} />
                <span>标记处理后记录将归档至该机台生命周期</span>
              </div>
              <div className='flex items-center space-x-4'>
                <button
                  onClick={() => setViewingInquiry(null)}
                  className='px-8 py-3 text-slate-600 font-black text-sm hover:text-slate-900 transition-colors'>
                  关闭
                </button>
                <button
                  onClick={() => {
                  ...
                  className='px-12 py-3.5 bg-blue-600 text-white rounded-2xl font-black text-sm shadow-2xl flex items-center active:scale-95 transition-all'>
                  <Save size={18} className='mr-2' /> 保存并结案
                </button>
              </div>
            </div>
          </div>
          ...
          <div className='bg-white w-full max-w-4xl h-[85vh] rounded-[3.5rem] shadow-2xl flex flex-col overflow-hidden border border-white/20'>
            <div className='px-10 py-8 border-b border-slate-100 flex items-center justify-between bg-white sticky top-0 z-10'>
              <div className='flex items-center space-x-5'>
                ...
                <div>
                  <h3 className='text-xl font-black text-slate-900'>
                    权限矩阵配置: {editingUser.name}
                  </h3>
                  <p className='text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1'>
                    成员 ID: {editingUser.id} · {editingUser.employeeId}
                  </p>
                </div>
              </div>
              <button
                  ...
                  <div className='space-y-4'>
                    ...
                    <div className='space-y-3'>
                      <div className='space-y-1'>
                        ...
                        <select
                          className='w-full p-3 bg-slate-50 rounded-xl border-none outline-none text-xs font-bold'
                          ...
                          <div className='flex space-x-1'>
                            {['none', 'view', 'manage'].map((p) => (
                              <button
                                ...
                                className={`px-2 py-1 rounded text-[8px] font-black uppercase ${value === p ? 'bg-indigo-600 text-white shadow-lg' : 'bg-white text-slate-400 border border-slate-200'}`}>
                                {p}
                              </button>
                            ))}
                          </div>
            ...
            <div className='px-10 py-6 border-t border-slate-100 flex items-center justify-end bg-white'>
              ...
              <button
                ...
                className='px-12 py-3.5 bg-indigo-600 text-white rounded-2xl font-black text-sm shadow-xl active:scale-95 transition-all'>
                <Save size={18} className='mr-2' /> 保存变更
              </button>
            </div>
          </div>
                        ...
                        {false && step.historyRepairCount !== undefined && (
                          <div className='flex items-center space-x-2 px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-xl'>
                            ...
                            <span className='text-[10px] font-black text-slate-500 uppercase tracking-tight'>
                              历史使用: {step.historyRepairCount} 次
                            </span>
                          </div>
                        )}
                      </div>
